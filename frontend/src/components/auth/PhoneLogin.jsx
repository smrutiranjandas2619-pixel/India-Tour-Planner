...
const PhoneLogin = () => {
  ...
  const validatePhone = (num) => {
    // Remove all non-digit characters except starting +
    const cleanNum = num.replace(/[^\d+]/g, '');
    
    // Check if it's already in +91XXXXXXXXXX format
  ...
  const handleVerifyOTP = async (otpCode) => {
    ...
    // Call backend endpoint to verify token and set session
    await loginWithPhone(null, idToken, false);
